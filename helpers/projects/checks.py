...
def check_project_pages_access_level(project):
    if project.project_attributes is not None:
        return {
            "project_pages_access_level": project.project_attributes[
                "pages_access_level"
            ]
        }
...
def check_project_security_and_compliance_enabled(project):
    if project.project_attributes is not None:
        return {
            "project_security_and_compliance_enabled": project.project_attributes[
...
def check_project_approvals_before_merge(project):
    if project.project_attributes is not None:
        return {
            "project_approvals_before_merge": project.project_attributes[
...
def check_project_push_rules_unsigned_commits(project):
    if project.project_push_rules is not None:
        return {
            "project_push_rules_unsigned_commits": project.project_push_rules.reject_unsigned_commits
        }
...
def check_project_push_rules_comitter_check(project):
    if project.project_push_rules is not None:
        return {
            "project_push_rules_comitter_check": project.project_push_rules.commit_committer_check
        }
...
def check_project_protected_branches(project):
    if project.project_protected_branches is not None:
        ...
        for i in project.project_protected_branches:
            ...
            try:
                project_protected_branches_list.append(
                    project_protected_branches._attrs
                )
            except:
...
def check_project_merged_pipeline_stages(project):
    if project.project_merged_pipeline is not None:
        pipeline_block_stages = project.get_project_merged_pipeline_block(
            project.project_merged_pipeline, "stages"
        )
...
def check_project_merged_pipeline_images(project):
    if project.project_merged_pipeline is not None:
        pipeline_block_images = project.get_project_merged_pipeline_content_of_block(
            project.project_merged_pipeline, "image"
        )

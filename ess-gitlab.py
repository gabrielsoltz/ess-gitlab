...
def check_project(gl_project):
    ...
    dict.update(check_project_file_pipeline(gl_project))
    dict.update(check_project_merged_pipeline(gl_project))
    dict.update(check_project_merged_pipeline_stages(gl_project))
    dict.update(check_project_merged_pipeline_images(gl_project))
    dict.update(check_project_file_codeowners(gl_project))
                        ...
                        for check, expected in value.items():
                            ...
                            if str(id) == "*":
                                ...
                                for scans_id in scan["ess-gitlab"][category]:
                                    try:
                                        project_name = scan["ess-gitlab"][category][
                                            scans_id
                                    ...
                                    try:
                                        project_url = scan["ess-gitlab"][category][
                                            scans_id
                                    ...
                                    if not scans_id in baseline_output_by_check_id:
                                        ...
                                        baseline_output_by_check_id[scans_id].append(
                                            {"name": project_name}
                                        )
                                        baseline_output_by_check_id[scans_id].append(
                                            {"url": project_url}
                                    ...
                                    logging.info(
                                        ...
                                        + " | Result: "
                                        + str(
                                            scan["ess-gitlab"][category][scans_id][
                                                check
                                            ]
                                        )
                                    )
                                    if expected != "None" and expected is not None:
                                        result = scan["ess-gitlab"][category][scans_id][
                                            check
                                        ]
                                        if (
                                            check_baseline_items(expected, result)
                                            == "ERROR"
                                        ):
                                            baseline_output_by_check.update(
                                                {check: "ERROR"}
                                            )
                                ...
                                try:
                                    project_url = scan["ess-gitlab"][category][str(id)][
                                        "project_info"
                                    ]["web_url"]
                                except:
                                    project_url = scan["ess-gitlab"][category][str(id)][
                                ...
                                if not id in baseline_output_by_check_id:
                                    ...
                                    baseline_output_by_check_id[id].append(
                                        {"name": project_name}
                                    )
                                    baseline_output_by_check_id[id].append(
                                        {"url": project_url}
                                ...
                                logging.info(
                                    ...
                                    + str(type(expected))
                                    + " | Result: "
                                    + str(scan["ess-gitlab"][category][str(id)][check])
                                )
                                if expected != "None" and expected is not None:
                                    result = scan["ess-gitlab"][category][str(id)][
                                        check
                                    ]
                                    if (
                                        check_baseline_items(expected, result)
    ...
    for projects, findings in baseline["ess-gitlab"]["baseline"]["projects"].items():
        ...
        if not totalonly:
            print(
                ...
                ")",
                "| FAILS:",
                total_fails_by_project,
                "| PASS:",
                total_pass_by_project,
                "| ERRORS:",
                total_errors_by_project,
                "| Checks: ",
                total_checks_by_project,
            )
    print(
        "Total Projects:",
        total_projects,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--id",
        nargs="*",
        help="Gitlab Project ID or Group ID or all, use spaces to add more than 1",
        required=True,
    )
    parser.add_argument(
        "-j",
    ...
    parser.add_argument(
        ...
        "--log",
        default="ERROR",
        help="Log Level (Default: ERROR) (Valid Options: ERROR, INFO or DEBUG)",
        required=False,
    )
    parser.add_argument(
        "-b",
        ...
        help="Baseline File (Default: baselines/default.yml)",
        required=False,
    )
    parser.add_argument(
        "-to",
        "--totalonly",
        action="store_true",
        help="Show total only (Default: No)",
        required=False,
    )
    parser.add_argument(
        "-a",
        "--scan-archived",
        action="store_true",
        help="Include archived projects (Default: No)",
        required=False,
    )
    parser.add_argument(
        "-ma",
        "--max-all",
        default=100,
        help="Maximun amount of projects when using all as id (Default: 100)",
        required=False,
    )
    parser.add_argument(
        "--scanlog4j",
    ...
    if args["gitlab_token"]:
        ...
        if gitlab_token == None:
            logging.error(
                "ERROR: No token found in environment, please check existence of 'gitlab_token' in current environment!"
            )
            sys.exit(1)
    ...
    if mode not in available_modes:
        logging.error(
            "ERROR: Wrong mode. Use: {}".format(",".join(map(str, available_modes)))
        )
        sys.exit(1)
    ...
    if check not in available_checks:
        logging.error(
            "ERROR: Wrong check. Use: {}".format(",".join(map(str, available_checks)))
        )
        sys.exit(1)
        ...
        for id in ids:
            ...
            for project in gl_group.projects_ids:
                ...
                print(
                    "Scanning Project:",
                    project,
                    str(count) + "/" + str(gl_group.projects_len) + "... ",
                )
